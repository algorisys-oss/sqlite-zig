...
## Current status: Phase 1 — 89 modules; core + rtree + session + FTS3 (complete) in Zig

**Modules 87–89 complete the FTS3 family** — full-text search is now 100% Zig:
`fts3_snippet.c` (snippet/offsets/matchinfo + sqlite3Fts3ExprIterate, byte-identical
to C), `fts3_tokenizer.c` (tokenizer registry), `fts3_tokenize_vtab.c` (fts3tokenize
vtab). All mirror fts3Int.h structs file-locally (no new offsets). Validated:
snippet([..]), offsets(), binary matchinfo(pcx), fts3tokenize('porter')→run/quickli/jump.

### THE ONLY REMAINING ACTIVE MODULE: fts5.c (IN PROGRESS — modular port)
fts5.c (28050 lines) is the FTS5 **amalgamation**. RE-VENDOR ATTEMPT FAILED: the
individual ext/fts5/*.c files are NOT standalone TUs (e.g. fts5_unicode2.c lacks
`#include "fts5Int.h"` and relies on concatenation order; static/extern + struct
fields differ standalone). SQLite only ever builds FTS5 as the single fts5.c
amalgamation. So we port the amalgamation as ONE Zig object built from modular
sub-files:
  - `src/fts5_int.zig` — FOUNDATION: byte-exact mirrors of every fts5Int.h struct
    (amalg lines 794-1754) + fts5.h public structs (38-793) + ALL FTS5 constants +
    rd/wr/off() helpers. Pure shared types; `@import`ed by every section.
  - `src/fts5_<section>.zig` — one per amalg section; each imports fts5_int.zig,
    `export fn`s its C-ABI symbols, `extern fn`s sibling symbols (resolved within
    the one object). Same idiom as the FTS3 family.
  - `src/fts5.zig` — ROOT: `comptime { _ = @import("fts5_<section>.zig"); … }` for
    every section, so all compile into the one swapped object. build.zig swaps
    "fts5.c" → src/fts5.zig.
  fts5parse stays generated C INSIDE the Zig? No — port fts5parse.c too (it's a
  Lemon table-driven parser; small) OR keep it... TBD; simplest is to port it as
  a section since the amalgamation has no separate parse TU to keep.
  Section line ranges in vendor/tsrc/fts5.c: fts5.h 38-793; fts5Int.h 794-1754;
  fts5parse 1755-3364; aux 3365-4187; buffer 4188-4600; config 4601-5727; expr
  5728-9015; hash 9016-9607; **index 9608-19169 (~9.5k, the crux: segment b-tree)**;
  main 19170-23071; storage 23072-24600; tokenize 24601-26098; unicode2
  26099-26880; varint 26881-27226; vocab 27227-28050.
  STATUS: foundation agent building fts5_int.zig + fts5_varint + fts5_buffer +
  the fts5.zig skeleton. Remaining sections to port after foundation lands:
  hash, unicode2, tokenize, config, aux, vocab, expr, index, storage, main,
  fts5parse. ~26k lines total — a multi-batch effort.

### Everything else (15 files) is NOT portable for a Linux engine:
- Generated: parse.c, opcodes.c (from parse.y / vdbe.c — never hand-ported).
- Other-platform: os_win.c, mutex_w32.c (Windows; dead on Linux).
- Flag-inactive (compile to nothing here): mem0/2/3.c, notify.c, os_kv.c, icu.c,
  fts3_icu.c, sqlite3rbu.c.
- TCL test harness: tclsqlite-ex.c (non_tu).

So: **the active Linux SQLite engine + RTree + Session + FTS3 are 100% Zig.**
FTS5 is the one remaining active extension.

## Earlier status: 86 modules; core + rtree + session + FTS3 core in Zig
