...
### THE ONLY REMAINING ACTIVE MODULE: fts5.c
fts5.c (28050 lines) is the FTS5 **amalgamation** — generated by concatenating
ext/fts5/*.c (fts5_main/index/storage/expr/config/tokenize/aux/buffer/hash/
unicode2/varint) + the Lemon-generated fts5parse.c. Per the project rule (never
hand-port generated files), the correct approach is to **re-vendor** FTS5 as its
~11 individual ext/fts5 source files (replace `fts5.c` in vendor/tu.txt + the
vendoring script), then port each file (Lemon fts5parse stays generated like
parse.c). This is a self-contained sub-project — the recommended next phase.
